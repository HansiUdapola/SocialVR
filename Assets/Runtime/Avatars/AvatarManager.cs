...
namespace Ubiq.Avatars
{
    ...
    {       
        ...
        private void Start()
        {
            ...
            //---- Updated by Hansi ----
            //load the previously saved prefab, if its empty then load default prefab given in Unity editor
            
            // if (LocalPrefabUuid.Length > 0)
            // {
            //     RoomClient.Me["ubiq.avatar.prefab"] = GetAvatarPrefabUuid() == "" ? LocalPrefabUuid : GetAvatarPrefabUuid();
            // }

            RoomClient.Me["ubiq.avatar.prefab"] = GetAvatarPrefabUuid();
            UpdateAvatar(RoomClient.Me);            
        }

        //Newly added to get saved settings of ubiq avatar prefab
        string GetAvatarPrefabUuid()
        {
            var uuid = PlayerPrefs.GetString("ubiq.avatar.prefab", "");
            return uuid;            
        }
...
//Newly added to updated the avatar body when call to change
        public void UpdateAvatarBodyprefab(IPeer peer){
            UpdateAvatar(peer);
        }
